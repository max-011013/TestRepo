...
def json_manipulation():
    try:
        ...
        df.to_json("_result.json", orient="records")
        ...
        for records in data:
            if (not records.get("accountNumber") or not records.get("accountName") or not records.get("address1")
                or not records.get("city") or not records.get("countryName") or not records.get("zipCode") or not records.get("state") 
...
def create_child_account(records,coll):
    try:
        #acc = coll.find_one({"accountNumber":records["accountNumber"]}) 
        #print(acc)
        if coll.find_one({"accountNumber":records["accountNumber"]}):
            print("Account already exist")
        else:
            ...
            if parent is not None and records["accountNumber"].isnumeric():
                records["relations"] = {
                    "childrenAccounts" : [
                    ],
                    "parentAccounts" : [
